...
class Tapper:
    ...
    async def get_tg_web_data(self) -> str:
        ...
        try:
            ...
            ref_id = choices([settings.REF_ID, "00006uXQ"], weights=[85, 15], k=1)[0]
    ...
    @error_handler
    async def check_trial_spins(self, http_client):
        return await self.make_request(http_client, "POST", "/spin/free", json={'language_code': 'en'})
    
    @error_handler
    async def make_trial_spins(self, http_client):
        return await self.make_request(http_client, "POST", "/spin/once")
    
    @error_handler
    async def check_free_spins(self, http_client):
        return await self.make_request(http_client, "POST", "/user/tickets", json={'language_code': 'en'})
    
    @error_handler
    async def make_free_spins(self, http_client, ticket: str):
        return await self.make_request(http_client, "POST", "/spin/raffle", json={'category': ticket})
        ...
        while True:
            try:
                ...
                if settings.AUTO_SPIN:
                    check_trial_spin = await self.check_trial_spins(http_client=http_client)
                    if check_trial_spin and check_trial_spin.get("status", 500) == 0:
                        if check_trial_spin.get('data', {}).get('is_free') is True:
                            logger.info(f"{self.session_name} | Found trial spin, Spinning...")
                            await asyncio.sleep(5)
                            trial_spin_result = await self.make_trial_spins(http_client=http_client)
                            if trial_spin_result and trial_spin_result.get("status", 500) == 0:
                                logger.info(f"{self.session_name} | Trial spin finished. Points: <light-red>+ {trial_spin_result['data']['results']['amount']} {trial_spin_result['data']['results']['type']}</light-red>")
                                
                    check_free_spin = await self.check_free_spins(http_client=http_client)
                    if check_free_spin and check_free_spin.get("status", 500) == 0:
                        free_tickets = check_free_spin.get('data', {}).get('ticket_spin_1')
                        if free_tickets not in (0, None):
                            logger.info(f"{self.session_name} | Found Spins Tickets: <light-red>{free_tickets}</light-red>, Spinning...")
                            await asyncio.sleep(5)
                            while free_tickets > 0:
                                make_spin = await self.make_free_spins(http_client=http_client, ticket='ticket_spin_1')
                                if make_spin and make_spin.get("status", 500) == 0:
                                    free_tickets -= 1
                                    results = make_spin.get('data', {}).get('results', [])
                                    if results:
                                        spin_points = f"{results[0]['amount']} {results[0].get('type', '⭐/🍅')}"
                                        logger.info(f"{self.session_name} | Spin Points: <light-red>{spin_points}</light-red>")
                                    else:
                                        logger.info(f"{self.session_name} | No results returned.")
                                    await asyncio.sleep(2)
                                else:
                                    if make_spin and make_spin.get('data') and 'message' in make_spin['data']:
                                        logger.info(f"{self.session_name} | Error Spinning: {make_spin['data']['message']}.")
                                    else:
                                        logger.info(f"{self.session_name} | Error Spinning.")
